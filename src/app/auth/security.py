...
def create_magic_tokens(
    *, subject: Union[str, Any], expires_delta: timedelta | None = None
    ...
    for sub in [subject, uuid.uuid4()]:
        to_encode = {"exp": expire, "sub": str(sub), "fingerprint": fingerprint}  # noqa
        encoded_jwt = jwt.encode(
            to_encode, settings.SECRET_KEY, algorithm=settings.JWT_ALGO
        )
...
def create_verification_pin() -> str:
    return str(uuid.uuid4())[:5]


def verify_verification_pin(*, db_obj: Any, pin: str) -> bool:
    return db_obj.verification_pin == pin
